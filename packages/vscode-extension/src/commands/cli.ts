...
export function registerCliCommands(context: vscode.ExtensionContext): void {
  ...
  const checkCliCommand = vscode.commands.registerCommand('wcai.checkCli', async () => {
    try {
      ...
      if (isAvailable) {
        ...
        vscode.window.showInformationMessage(`✅ WCAI CLI is available${version ? ` (${version})` : ''}`);
      } else {
  ...
  const installCliCommand = vscode.commands.registerCommand('wcai.installCli', async () => {
    ...
    vscode.window.showInformationMessage('Installing WCAI CLI globally. Check the terminal for progress.');
  });
    ...
    vscode.window
      ...
      .then(action => {
        if (action === 'Restart Server') {
          vscode.commands.executeCommand('wcai.mcp.restart');
        }
      });
  });
  ...
  const runSetupCommand = vscode.commands.registerCommand('wcai.runCliSetup', async () => {
    ...
    vscode.window.showInformationMessage('Running WCAI setup wizard. Check the terminal for interactive prompts.');
  });
